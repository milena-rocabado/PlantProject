...
void global_process(const Mat &src, Mat &dst) {
    ...
    // Umbralizar
    threshold(dst, dst, GLOBAL_THRESHOLD, 255, THRESH_BINARY);
    if (dump)
        save("Global/" + to_string(pos) + "_thr", dst);
}

void run_global_segmentator() {
    VideoCapture video(string(WD) + "climbing_bean_project3_leaf_folding.AVI");
    ...
    VideoWriter salida(string(WD) + "/global_dump_out.AVI", static_cast<int>(video.get(CAP_PROP_FOURCC)),
